...
export default class Css {
  ...
  static getRandomUnitRule() {
    return `${Css.getRandomUnitProp()}: ${Helpers.getRandomInt(0, 500)}${Css.getRandomUnit()}`;
  }
  ...
  static getRandomClassName() {
    ...
    return `${Helpers.getRandomEntry(options)}__${Helpers.getRandomEntry(options)}`;
  }
  ...
  static getRandomAlign() {
    ...
    return `align-${Helpers.getRandomEntry(alignTypes)}: ${Helpers.getRandomEntry(alignValues)}`;
  }
  ...
  static getRandomBackgroundPostition() {
    ...
    return `background-position: ${Helpers.getRandomEntry(backgroundPositionValues)}`;
  }
  ...
  static getRandomBackgroundRepeat() {
    ...
    return `background-repeat: ${Helpers.getRandomEntry(backgroundRepeatValues)}`;
  }
  ...
  static getRandomBoxShadow() {
    let boxShadowValue = `${Helpers.getRandomInt(
      1,
      10
    )}${this.getRandomUnit()} ${Helpers.getRandomInt(1, 10)}${this.getRandomUnit()} `;
    ...
    if (Helpers.getRandomInt(1, 100) <= 50) {
      boxShadowValue += `${Helpers.getRandomInt(
        2,
        20
      )}${this.getRandomUnit()} ${Helpers.getRandomInt(2, 20)}${this.getRandomUnit()} `;
    }
  ...
  static generateRandomCode(lines, addComment) {
    ...
    fillerLines = fillerLines.concat(
      Array(fillerLineQty)
        .fill()
        .map((l) => `${Helpers.getIndentation(2)}${Helpers.getRandomEntry(lineOptions)};`)
    );
